...
from dreamidv_wan.utils.utils import cache_video, str2bool
...
def _parse_args():
    ...
    parser.add_argument(
        "--task", type=str, default="swapface", choices=list(WAN_CONFIGS.keys()), help="The task to run."
    )
    parser.add_argument(
        "--size",
        type=str,
    ...
    parser.add_argument("--sample_fps", type=int, default=24, help="The fps of the generated video.")
    parser.add_argument("--ckpt_dir", type=str, default=None, help="The path to the checkpoint directory.")
    parser.add_argument("--dreamidv_ckpt", type=str, default=None, help="The path to the Phantom-Wan checkpoint.")
    parser.add_argument(
        "--offload_model",
        type=str2bool,
        default=None,
    ...
    parser.add_argument(
        "--ref_video_facemask",
        type=str,
        default="./assets/test_case/ref_video_facemask/a_girl.mp4",
        help="The reference video facemask.",
    )
    parser.add_argument(
        "--ref_video_pose",
        type=str,
        default="./assets/test_case/ref_video_pose/a_girl.mp4",
        help="The reference video pose.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--sample_guide_scale_img", type=float, default=4.0, help="Classifier free guidance scale for reference images."
    )
...
def generate(args):
    ...
    ref_mask_path = args.ref_video_facemask
    ref_img_path = args.ref_image
    ref_pose_path = args.ref_video_pose

    text_prompt = prompt

    ref_paths = [ref_video_path, ref_mask_path, ref_img_path, ref_pose_path]
    ...
    if rank == 0:
        if args.save_file is None:
            ...
            args.save_file = (
                f"./results/{args.task}_{args.size}_{args.ulysses_size}_{args.ring_size}_{formatted_time}" + suffix
            )
        cache_video(
            tensor=video[None],
