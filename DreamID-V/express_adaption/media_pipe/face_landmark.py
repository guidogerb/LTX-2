...
import numpy as np
from mediapipe.framework.formats import classification_pb2, landmark_pb2, matrix_data_pb2
...
def _build_landmarker_result(output_packets: Mapping[str, packet_module.Packet]) -> FaceLandmarkerResult:
    """Constructs a `FaceLandmarkerResult` from output packets."""
    face_landmarks_proto_list = packet_getter.get_proto_list(output_packets[_NORM_LANDMARKS_STREAM_NAME])
    ...
    for proto in face_landmarks_proto_list:
        ...
        for face_landmark in face_landmarks.landmark:
            face_landmarks_list.append(landmark_module.NormalizedLandmark.create_from_pb2(face_landmark))
        face_landmarks_results.append(face_landmarks_list)

    face_blendshapes_results = []
    if _BLENDSHAPES_STREAM_NAME in output_packets:
        face_blendshapes_proto_list = packet_getter.get_proto_list(output_packets[_BLENDSHAPES_STREAM_NAME])
...
def _build_landmarker_result2(output_packets: Mapping[str, packet_module.Packet]) -> FaceLandmarkerResult:
    """Constructs a `FaceLandmarkerResult` from output packets."""
    face_landmarks_proto_list = packet_getter.get_proto_list(output_packets[_NORM_LANDMARKS_STREAM_NAME])
...
class FaceLandmarkerOptions:
    ...
    def to_pb2(self) -> _FaceLandmarkerGraphOptionsProto:
        ...
        face_landmarker_options_proto = _FaceLandmarkerGraphOptionsProto(base_options=base_options_proto)
...
class FaceLandmarker(base_vision_task_api.BaseVisionTaskApi):
    ...
    def create_from_model_path(cls, model_path: str) -> "FaceLandmarker":
        ...
        options = FaceLandmarkerOptions(base_options=base_options, running_mode=_RunningMode.IMAGE)
    ...
    def create_from_options(cls, options: FaceLandmarkerOptions) -> "FaceLandmarker":
        ...
        if options.output_face_blendshapes:
            output_streams.append(":".join([_BLENDSHAPES_TAG, _BLENDSHAPES_STREAM_NAME]))
        if options.output_facial_transformation_matrixes:
            output_streams.append(":".join([_FACE_GEOMETRY_TAG, _FACE_GEOMETRY_STREAM_NAME]))
        ...
        return cls(
            task_info.generate_graph_config(enable_flow_limiting=options.running_mode == _RunningMode.LIVE_STREAM),
    ...
    def detect(
        ...
        normalized_rect = self.convert_to_normalized_rect(image_processing_options, image, roi_allowed=False)
        output_packets = self._process_image_data(
            {
                _IMAGE_IN_STREAM_NAME: packet_creator.create_image(image),
                _NORM_RECT_STREAM_NAME: packet_creator.create_proto(normalized_rect.to_pb2()),
            }
        )
        ...
        output_packets = self._process_video_data(
            {
                ...
                _NORM_RECT_STREAM_NAME: packet_creator.create_proto(normalized_rect.to_pb2()).at(
                    timestamp_ms * _MICRO_SECONDS_PER_MILLISECOND
                ),
            }
        )
