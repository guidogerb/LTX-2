...
def usp_dit_forward(
    ...
    pose_embedding = torch.chunk(pose_embedding, sp_world, dim=1)[sp_rank]
...
def usp_attn_forward(self, x, seq_lens, grid_sizes, freqs, pose_embedding=None, dtype=torch.bfloat16):
    ...
    x_out = xFuserLongContextAttention()(None, query=half(q), key=half(k), value=half(v))
