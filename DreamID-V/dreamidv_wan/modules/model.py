...
def rope_params(max_seq_len, dim, theta=10000):
    assert dim % 2 == 0
    freqs = torch.outer(torch.arange(max_seq_len), 1.0 / torch.pow(theta, torch.arange(0, dim, 2).div(dim)))
...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs, pose_embedding=None):
        ...
        x_out = flash_attention(q=q_roped, k=k_roped, v=v, k_lens=seq_lens, window_size=self.window_size)
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        # self-attention
        y = self.self_attn(
            self.norm1(x).float() * (1 + e[1]) + e[0], seq_lens, grid_sizes, freqs, pose_embedding=pose_embedding
        )
        with amp.autocast(dtype=torch.float32):
            x = x + y * e[2]
