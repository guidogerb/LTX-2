...
def mock_deps():
    ...
    with patch("vtx_app.registry.db.Registry") as MockRegistrySource, \
         ...
         patch("vtx_app.wizards.proposal.ProposalGenerator") as MockProposalGen, \
         patch("vtx_app.producer.Director") as MockDirector:
         
         # Apply these mocks to vtx_app.cli globals as well
         with patch("vtx_app.cli.Registry", MockRegistrySource), \
...
def test_render_reviews(mock_deps):
    ...
    with patch("pathlib.Path.exists") as mock_exists, \
        ...
        mock_deps["renderer"].render_clip.assert_called_with(
            clip_id="c1", resolution_scale=0.5, output_dir=proj.root / "renders" / "low-res"
        )
...
def test_create_movie_all(mock_deps):
     ...
     with runner.isolated_filesystem():
        ...
        with patch.object(Path, "glob", return_value=[Path("c1.yaml")]), \
...
def test_render_reviews_exception(mock_deps):
     with patch("pathlib.Path.exists", return_value=True), \
         patch("pathlib.Path.glob", return_value=[Path("c1.yaml")]):
...
def test_render_approve_warning(mock_deps):
    ...
    with patch("pathlib.Path.exists", autospec=True, side_effect=side_effect) as mock_exists, \
         patch("pathlib.Path.read_text", return_value="render: {strategy: v2v}\noutputs: {mp4: out.mp4}"), \
